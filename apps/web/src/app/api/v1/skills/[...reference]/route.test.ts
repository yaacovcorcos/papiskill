...
vi.mock("@/lib/server/catalog", () => ({
  getCatalogSkills: mocks.getCatalogSkills,
...
describe("GET /api/v1/skills/[...reference]", () => {
  beforeEach(() => {
    ...
    mocks.getFileRegistrySkill.mockResolvedValue(null);
    mocks.getCatalogSkills.mockResolvedValue([catalogSkill]);
  });
  ...
  it("does not fall back from profile references to public skills with the same slug", async () => {
    ...
    expect(mocks.getCatalogSkills).not.toHaveBeenCalled();
  });
});
